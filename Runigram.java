...
public class Runigram {
	...
	private static Color luminance(Color pixel) {
		...
		Color lPixel = new Color(lum, lum, lum);
		return lPixel;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		if (image != null) {
			...
			double alphaSizeHeight = (double) image.length / height; // h0/h
			double alphaSizeWidth = (double) image[0].length / width; // w0/w
			for (int i = 0; i < height; i++) {
				for (int j = 0; j < width; j++) {
					int nh = (int) (i * alphaSizeHeight); // i*(j0/h)
					int nw = (int) (j * alphaSizeWidth); // j*(w0/w)
					resizeImage[i][j] = image[nh][nw];
				}
			}
			return resizeImage;
		}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		//// Replace this comment with your code
		Color[][] morphSrc = scaled(target, source[0].length, source.length);
		for (int s = 0; s <= n; s++) { // s=steps
			double alpha = (double) s / n;
			Color[][] animation = blend(source, morphSrc, alpha);
			display(animation);
			StdDraw.pause(300);
		}
	}
